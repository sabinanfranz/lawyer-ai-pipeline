...
## OpenAI 모드 (선택)
- `.env`: `LLM_MODE=openai`, `OPENAI_API_KEY` 설정 (모델/timeout 옵션은 .env 참고)
- `npm run dev`
...
## 트러블슈팅
- `DEBUG_AGENT=1`로 실행하면 env 판단, LLM 요청/에러, jsonGuard repair, cache hit/miss, fallback 이유(LLM_ERROR/JSON_GUARD_FALLBACK) 로그 확인 가능
